<?php
// src/Api/ayudantes/ApiGetAyudantes.php
header("Content-Type: application/json");
header("Access-Control-Allow-Origin: *");
header("Access-Control-Allow-Methods: POST");
header("Access-Control-Allow-Headers: Content-Type");

if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    http_response_code(405);
    echo json_encode(["success" => false, "message" => "Método no permitido"]);
    exit;
}

include $_SERVER['DOCUMENT_ROOT'] . "/DatenBankenApp/AllSeasonFlowers/conexionBaseDatos/conexionbd.php";

if ($enlace->connect_error) {
    http_response_code(500);
    echo json_encode(["success" => false, "message" => "Error de conexión: " . $enlace->connect_error]);
    exit;
}

$json = file_get_contents("php://input");
$filtros = json_decode($json, true) ?? [];

try {
    $sql = "SELECT * FROM GEN_Ayudantes WHERE 1=1";
    
    // Aplicar filtros
    if (isset($filtros["busqueda"]) && !empty($filtros["busqueda"])) {
        $busqueda = $enlace->real_escape_string($filtros["busqueda"]);
        $sql .= " AND (NomAyudante LIKE '%$busqueda%' 
                      OR NoCedula LIKE '%$busqueda%')";
    }
    
    if (isset($filtros["estado"]) && $filtros["estado"] !== "todos") {
        if ($filtros["estado"] === "activos") {
            $sql .= " AND ACTIVO = 1";
        } elseif ($filtros["estado"] === "inactivos") {
            $sql .= " AND ACTIVO = 0";
        }
    }
    
    $sql .= " ORDER BY NomAyudante ASC";
    
    $result = $enlace->query($sql);
    
    if (!$result) {
        throw new Exception("Error en consulta: " . $enlace->error);
    }
    
    $ayudantes = [];
    while ($row = $result->fetch_assoc()) {
        $row["ACTIVO"] = $row["ACTIVO"] == 1;
        $ayudantes[] = $row;
    }
    
    // Estadísticas
    $sqlStats = "SELECT 
        COUNT(*) as total,
        SUM(ACTIVO = 1) as activos,
        SUM(ACTIVO = 0) as inactivos
        FROM GEN_Ayudantes";
    
    $resultStats = $enlace->query($sqlStats);
    $estadisticas = $resultStats->fetch_assoc() ?? ["total" => 0, "activos" => 0, "inactivos" => 0];
    
    echo json_encode([
        "success" => true,
        "ayudantes" => $ayudantes,
        "estadisticas" => $estadisticas,
        "total" => count($ayudantes)
    ]);
    
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode([
        "success" => false,
        "message" => "Error al obtener ayudantes: " . $e->getMessage()
    ]);
}

$enlace->close();
?>
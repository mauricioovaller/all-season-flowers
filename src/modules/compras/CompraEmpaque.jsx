// src/modules/compras/CompraEmpaque.jsx
import React, { useState, useEffect, useMemo, useCallback } from "react";
import EmpaqueItemCompra from "./EmpaqueItemCompra";

// Colores diferenciados para cada empaque (mismo que pedidos)
...
export default function CompraEmpaque({
  empaques,
  onChangeEmpaques,
  productos = [],
  tiposEmpaque = [],
  ...
  return (
    <section className="bg-white rounded-lg md:rounded-xl shadow-sm md:shadow-md p-3 md:p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 gap-2">
        <div>
          <h3 className="text-base md:text-lg font-semibold text-slate-700">Empaques de Compra</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4 p-2 bg-gray-50 rounded-lg border">
        ...
        <div className="text-center">
          <div className="text-xs text-gray-600 font-medium">Valor Compra</div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {itemsFiltrados.map((plano, i) => {
                  ...
                  return (
                    <tr key={plano.key} className={`border-t border-gray-100 hover:bg-orange-50/50 transition ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                      ...
                      <td className="px-2 py-1.5 text-right">{formatCurrency(item.precioCompra, esUSD)}</td>
            ...
            {itemsFiltrados.map((plano) => {
              ...
              <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-xs text-gray-600">
                ...
                <span>T/Caja: <strong>{formatNumber((item.tallosRamo || 0) * (item.ramosCaja || 0))}</strong></span>
                    <span>Precio: <strong>{formatCurrency(item.precioCompra, esUSD)}</strong></span>

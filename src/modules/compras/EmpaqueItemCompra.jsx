// src/modules/compras/EmpaqueItemCompra.jsx
import React, { useState, useEffect, useCallback } from "react";
import { getVariedadesYGrados } from "../../services/compras/comprasService";
...
export default function EmpaqueItemCompra({
  empaqueIndex,
  items,
  onChangeItems,
  productos = [],
  ...
  }, [items, cantidadEmpaque, empaqueExpandido]);
  ...
  function addItem() {
    ...
    const newItem = {
      ...
      ramosCaja: 0,
      tallosCaja: 0,
      precioCompra: 0, // CAMBIADO: precioCompra en lugar de precioVenta
      totTallosRegistro: 0,
      valorRegistro: 0,
      predio: "",
      esBouquet: false,
  ...
  function updateItem(index, field, value) {
    ...
    } else if (field === "precioCompra") { // CAMBIADO: precioCompra
  ...
  function recalcularItem(item, cantEmpaqueFisico) {
    ...
    // Convertir precioCompra a número (maneja decimales con punto o coma)
    const precioStr = String(item.precioCompra || "0");
    ...
    return {
      ...
      precioCompra: item.precioCompra, // Mantener como string para input (CAMBIADO)
      tallosCaja,
      totTallosRegistro,
      valorRegistro,
    };
      ...
      if (item.esBouquet && item.receta) {
        item.receta.forEach((ingrediente, ingIndex) => {
          if (ingrediente.producto && ingrediente.variedad) {
            ...
            // Puedes resetear la variedad si no existe (opcional)
            // if (!variedadExiste && variedadesDisponibles.length > 0) {
            //   item.receta[ingIndex].variedad = "";
            //   huboCambios = true;
            // }
          }
        });
              ...
              {estaItemExpandido && (
                <div className="p-2 space-y-2">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-1.5">
                    ...
                    {/* Precio Compra - VERSIÓN SIMPLE QUE SÍ FUNCIONA */}
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-0.5">Precio Compra *</label>
                      <input
                        type="text"
                        value={item.precioCompra || ""}
                        onChange={(e) => {
                          updateItem(idx, "precioCompra", e.target.value);
                        }}

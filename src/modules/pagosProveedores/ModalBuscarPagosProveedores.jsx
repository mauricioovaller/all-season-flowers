// src/modules/pagosProveedores/ModalBuscarPagosProveedores.jsx
import React, { useState, useEffect, useCallback } from "react";
import Swal from "sweetalert2";
import { getDatosSelectCompras } from "../../services/compras/comprasService";
import { buscarPagosProveedores, getMediosPago } from "../../services/pagosProveedores/pagosProveedoresService";

const ModalBuscarPagosProveedores = ({ isOpen, onClose, onSeleccionarPago }) => {
  ...
  useEffect(() => {
    async function cargarDatos() {
      try {
        ...
        const mediosPagoRes = await getMediosPago();
        setMediosPago(mediosPagoRes.mediosPago || []);
      } catch (err) {
  ...
  const buscarPagos = useCallback(async () => {
    try {
      setLoading(true);
      const res = await buscarPagosProveedores(filtros);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleFiltroChange("idProveedor", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleFiltroChange("idMedioPago", e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {pagos.map((pago) => (
                      <tr key={pago.idPagoProveedor} className="hover:bg-gray-50">
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">
                          {pago.numeroPago}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                          {new Date(pago.fechaPago + 'T00:00:00').toLocaleDateString('es-CO')}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                          {pago.proveedor}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                          {pago.cantidadCompras} compra{pago.cantidadCompras !== 1 ? 's' : ''}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                          {pago.medioPago}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                          ${pago.valorPago.toLocaleString('es-CO', { minimumFractionDigits: 2 })}
                        </td>
...
export default ModalBuscarPagosProveedores;
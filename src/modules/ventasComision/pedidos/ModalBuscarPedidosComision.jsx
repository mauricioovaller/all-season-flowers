...
import { Search, X, FileText, Copy } from 'lucide-react';
import { buscarPedidos } from '../../../services/ventasComision/pedidosComisionService';

export default function ModalBuscarPedidosComision({ onClose, onSelect, onDuplicate }) {
  ...
  const handleBuscar = async (pagina = 1) => {
    setCargando(true);
    const filtrosBusqueda = { ...filtros, pagina, itemsPorPagina: itemsPerPage };
    const data = await buscarPedidos(filtrosBusqueda);
    if (data.success) {
      setResultados(data.pedidos || []);
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full" onClick={e => e.stopPropagation()}>
        ...
        <div className="bg-gradient-to-r from-violet-700 to-purple-700 px-6 py-4 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-white font-bold">Buscar Pedidos</h3>
              <p className="text-purple-200 text-sm">Busque y seleccione un pedido para editar o duplicar</p>
            </div>
          </div>
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">N° Pedido</label>
        ...
        <div className="p-6 max-h-96 overflow-y-auto">
          {cargando ? (
            ...
            <div className="space-y-2">
              {resultados.map(r => (
                <div key={r.idPedido} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-200 hover:border-violet-300 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="p-2 bg-violet-100 rounded-lg">
                      <FileText className="w-4 h-4 text-violet-600" />
                    </div>
                    <div>
                      <p className="font-semibold text-gray-800 text-sm">{r.numeroPedido || `ID: ${r.idPedido}`}</p>
                      <p className="text-xs text-gray-500">{r.cliente || 'Sin cliente'}</p>
                      {(r.fechaSolicitud || r.fecha) && (
                        <p className="text-xs text-gray-400">{r.fechaSolicitud || r.fecha}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-semibold px-2 py-1 rounded-full ${r.estado === 'Anulado' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>
                      {r.estado || 'Activo'}
                    </span>
                    ...
                    <button onClick={() => onDuplicate(r)}
                      className="p-1.5 text-gray-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Duplicar">
        ...
        {totalPaginas > 1 && (
          <div className="px-6 pb-4 flex items-center justify-between text-sm">
            <p className="text-gray-500">Total: {total} pedidos</p>
            <div className="flex items-center gap-2">

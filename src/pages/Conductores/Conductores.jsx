// src/pages/Conductores/Conductores.jsx
import React, { useState, useEffect } from 'react';
import Swal from 'sweetalert2';
import {
    Truck, Plus, Search, Edit, Filter,
    Download, RefreshCw, CheckCircle, XCircle, User, CreditCard, Phone, Car, Hash
} from 'lucide-react';

// Componentes
import ConductoresForm from './ConductoresForm';
...
const Conductores = () => {
    ...
    useEffect(() => {
        if (mostrarListado && conductores.length === 0) {
            cargarConductores();
        }
    }, [mostrarListado]);
    ...
    const cargarConductores = async () => {
        try {
            ...
            if (data.success) {
                const conductoresCorregidos = data.conductores.map(conductor => ({
                    ...conductor,
                    ACTIVO: conductor.ACTIVO === true ? 1 : 0
                }));

                setConductores(conductoresCorregidos || []);
                ...
                Swal.fire({
                    icon: 'warning',
                    title: 'Advertencia',
                    text: data.message || 'No se pudieron cargar los conductores',
                    timer: 3000
                });
                setConductores([]);
            }
    ...
    const calcularEstadisticas = () => {
        ...
        const activos = conductores.filter(c => c.ACTIVO === 1 || c.ACTIVO === true).length;
        const inactivos = total - activos;

        setEstadisticas({ total, activos, inactivos });
    };

    const handleGuardarConductor = async (conductorData) => {
        try {
            const datosCompletos = {
                ...conductorData,
                IdConductor: conductorEditando ? parseInt(conductorEditando.IdConductor) : undefined
            };

            const resultado = await guardarConductor(datosCompletos);

            if (resultado.success) {
                Swal.fire({
                    icon: 'success',
                    title: conductorEditando ? 'Conductor actualizado' : 'Conductor guardado',
                    html: `
          <div class="text-left">
            <p><strong>Conductor:</strong> ${conductorData.NombreConductor}</p>
            <p><strong>Cédula:</strong> ${conductorData.NoCedula}</p>
            <p><strong>Placas:</strong> ${conductorData.Placas}</p>
            <p><strong>ID:</strong> ${resultado.idConductor}</p>
        ...
        `,
            ...
            Swal.fire({
                ...
                html: `
        <div class="text-left">
          <p class="font-semibold">No se pudo guardar el conductor</p>
    ...
    const handleEditarConductor = (conductor) => {
        ...
        setTimeout(() => {
            document.getElementById('formulario-conductores')?.scrollIntoView({
                behavior: 'smooth',
                block: 'start'
            });
        }, 100);
    ...
    const conductoresFiltrados = conductores.filter(conductor => {
        const coincideBusqueda =
            !busqueda ||
            conductor.NombreConductor.toLowerCase().includes(busqueda.toLowerCase()) ||
            conductor.NoCedula.includes(busqueda) ||
            conductor.Telefono.includes(busqueda) ||
            conductor.Placas.toLowerCase().includes(busqueda.toLowerCase()) ||
            conductor.TipoVehiculo?.toLowerCase().includes(busqueda.toLowerCase()) ||
            conductor.Marca?.toLowerCase().includes(busqueda.toLowerCase());

        const coincideEstado =
            filtroEstado === 'todos' ||
            (filtroEstado === 'activos' && (conductor.ACTIVO === 1 || conductor.ACTIVO === true)) ||
            (filtroEstado === 'inactivos' && (conductor.ACTIVO === 0 || conductor.ACTIVO === false));
        ...
        <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-gray-600 mt-2">
                            Administra la información de los conductores de All Season Flowers
                        </p>
                    </div>
                    ...
                    >
                        ...
                        <span className="relative">NUEVO CONDUCTOR</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-blue-700">Total Conductores</p>
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-xl p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-green-700">Conductores Activos</p>
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-1">
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder="Buscar por nombre, cédula, teléfono, placas, vehículo..."
                                value={busqueda}
            ...
            {!mostrarListado && (
                <div className="text-center py-8">
                    ...
                    >
                        <Truck className="w-6 h-6" />
                        VER CONDUCTORES EXISTENTES
                    </button>
                    <p className="text-gray-500 mt-3">Mostrar listado completo de conductores registrados</p>
                </div>
            )}

            {/* ========== FORMULARIO (COLAPSABLE) ========== */}
            {mostrarFormulario && (
                <div id="formulario-conductores" className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    <div className="p-6">
                    ...
                    <div className="p-6">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-gray-800">
                                ...
                                <span className="text-sm font-normal text-gray-500 ml-2">
                                    ({conductoresFiltrados.length} de {conductores.length})
                                </span>
                            </h2>
                            <button
                        ...
                        {loading ? (
                            ...
                            <ConductoresList
                                ...
                                onEditar={handleEditarConductor}                                
                            />
                        )}
                    </div>
                </div>
            ...
            <div className="bg-gradient-to-r from-primary/5 to-secondary/5 rounded-2xl border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                    💡 Tips de Gestión de Conductores
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white p-4 rounded-lg border border-gray-100">
                        <div className="text-primary font-bold text-sm mb-2">✓ Datos Únicos</div>
                        <p className="text-sm text-gray-600">Nombre, cédula y placas deben ser únicos en el sistema.</p>
                    </div>
                    <div className="bg-white p-4 rounded-lg border border-gray-100">
                        <div className="text-primary font-bold text-sm mb-2">✓ Información del Vehículo</div>
                        <p className="text-sm text-gray-600">Complete tipo, marca, color y placas del vehículo asignado.</p>
                    </div>
                    <div className="bg-white p-4 rounded-lg border border-gray-100">
                        <div className="text-primary font-bold text-sm mb-2">✓ Estado Activo/Inactivo</div>
                        <p className="text-sm text-gray-600">Los conductores inactivos no estarán disponibles para asignaciones.</p>
                    </div>
                </div>
            </div>
        </div>

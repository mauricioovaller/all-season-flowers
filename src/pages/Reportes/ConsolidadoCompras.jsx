...
import { getConsolidadoCompras } from '../../services/reportes/reportesService';
...
const ConsolidadoCompras = () => {
  ...
  const consultar = async () => {
    ...
    const data = await getConsolidadoCompras({ fechaInicio, fechaFin });
    if (!data.success) {
      setError(data.message || 'Error al consultar consolidado de compras');
    } else {
      setResultado(data);
    }
    setLoading(false);
  ...
  const columnas = [
    ...
    { key: 'Total Compra', label: 'Total Compra' },
  ];
  ...
  const obtenerValor = (r, i, key) => {
    switch (key) {
      ...
      case 'Fecha Compra': return r.fechaCompra;
      ...
      case 'Total Compra': return String(r.totalCompra);
      default: return '';
    }
  };
  ...
  const totalesFiltrados = useMemo(() => {
    ...
    const totalCompra = registrosFiltrados.reduce((sum, r) => sum + (Number(r.totalCompra) || 0), 0);
    return { cantidadRegistros, totalTallos, subtotal, valorIVA, totalCompra };
  ...
  const exportarExcel = () => {
    ...
    const colWidths = [
      { wch: 5 }, { wch: 30 }, { wch: 14 }, { wch: 15 },
      { wch: 22 }, { wch: 18 }, { wch: 12 }, { wch: 14 },
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Consolidado Compras');
    XLSX.writeFile(wb, `ConsolidadoCompras_${fechaInicio}_${fechaFin}.xlsx`);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-800">Consolidado de Compras</h1>
          <p className="text-sm text-gray-500">
            Reporte detallado de compras por rango de fechas
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        ...
        <div className="flex gap-3 mt-4">
          <button
            onClick={consultar}
            disabled={loading}
            className="flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white px-5 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
          >
            {loading
      ...
      {resultado && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="bg-orange-50 border-b border-orange-200 px-5 py-3 flex flex-wrap justify-between items-center gap-2">
            <div>
              <span className="font-semibold text-orange-800 text-sm">Consolidado de Compras</span>
          ...
          {(filtroActivo ? registrosFiltrados : registros).length === 0 ? (
            <div className="p-10 text-center text-gray-400 text-sm">
              {filtroActivo ? 'No hay registros que coincidan con los filtros aplicados.' : 'No hay compras para el período seleccionado.'}
            </div>
          ) : (
            <>
              {totalesFiltrados && (
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 p-4 bg-orange-50/50 border-b border-orange-100">
                  <div className="bg-white rounded-lg p-3 text-center border border-orange-200">
                    ...
                    <p className="text-lg font-bold text-orange-700">{totalesFiltrados.cantidadRegistros}</p>
                  </div>
                  <div className="bg-white rounded-lg p-3 text-center border border-orange-200">
                    <p className="text-xs text-gray-500 uppercase font-medium">Total Tallos</p>
                    <p className="text-lg font-bold text-orange-700">{Number(totalesFiltrados.totalTallos || 0).toLocaleString('es-CO')}</p>
                  </div>
                  <div className="bg-white rounded-lg p-3 text-center border border-orange-200">
                  ...
                  <div className="bg-white rounded-lg p-3 text-center border border-orange-200">
                    <p className="text-xs text-gray-500 uppercase font-medium">Total Compras</p>
                    <p className="text-lg font-bold text-green-700">{formatNum(totalesFiltrados.totalCompra)}</p>
                  </div>
                </div>
              )}
                      ...
                      {columnas.map(col => (
                        <th key={col.key} className="px-1 py-1">
                          <input
                            ...
                            placeholder="..."
                            className="w-full h-6 px-1 text-[10px] border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-orange-400"
                          />
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {registrosFiltrados.map((r, i) => (
                      <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        <td className={tdClass}>{r.fechaCompra}</td>
                        ...
                        <td className={tdClass + ' text-right font-bold text-green-700'}>{formatNum(r.totalCompra)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <div key={i} className={`p-4 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500">{r.fechaCompra} | PO: {r.po}</p>
                      </div>
                      <span className="text-sm font-bold text-green-700 flex-shrink-0 ml-2">{formatNum(r.totalCompra)}</span>
                    </div>
...
export default ConsolidadoCompras;
